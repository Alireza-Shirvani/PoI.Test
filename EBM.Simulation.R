###############The R code for the exponential Brownian motion
#chooseCRANmirror(47)
...
b           <- 1
domain      <- c(a,b)
#F is used instead of infty
F=100
## Define Parameters for Simulation 
B          <-  2000
# B is the number of replications
maxPoI      <- 8    # maximum number of allowed points of impact


###########################################################################
#############################################################################

## Threshold for matching tau_s with tau^hat_s
threshold    <- 0.01
## Simulation for Complicated with different noise-to-signal ratios
sigmas      <- c(0.71,1,1.41)
for(p in gridpoints){
  maxK <- p/1.5
  grd   <- seq(a,b, length.out = p) # Grid for X-values
  k_seq       <- if (p == 300) {
    c(1:10, seq(12, 20, 2), seq(25, 60, 5)) 
  } else if (p == 500){
    c(1, seq(2, 20, 2), seq(25, 50, 5), seq(60, 100, 10))
  } else if (p == 1000){
    c(1, seq(2, 20, 2), seq(25, 50, 5), seq(60, 150, 10))  
  }
  for (sigma in sigmas){
    
    scaleSearchPoI <- TRUE
    scalePoISearch <- as.character(sigma) ## dirty hack... !!!!
    for(N in samplesize){
      for(DGP_name in DGP){
 ...
 ############################################################
 ### beta is the slope function evaluated at s
###########################################################3
# Set parameters for the simulation
n <- 100 # number of basis functions
grid_size <- 1000 # resolution for discretization

# Define the kernel function
kernel <- function(t, s) {
  exp((t + s) / 2)*(exp(min(t,s)) - 1)
}

# Discretize the space
t_grid <- seq(0, 1, length.out = grid_size)
s_grid <- seq(0, 1, length.out = grid_size)
K_matrix <- outer(t_grid, s_grid, Vectorize(kernel))

# Solve the eigenproblem numerically
eigen_result <- eigen(K_matrix, symmetric = TRUE)

# Extract eigenvalues and eigenvectors
eigenvalues <- eigen_result$values[1:n]
eigenvectors <- eigen_result$vectors[, 1:n]

# Normalize eigenvectors to ensure orthonormality
eigenvectors <- apply(eigenvectors, 2, function(x) {
  x / sqrt(sum(x^2))
})

# Generating the beta function using the eigen-decomposition
beta <- function(s,q) {
  beta_vals <- numeric(length(s))
  for (j in 1:n) {
    # Since simulating in continuous form, we create an approximating function
    # It interpolates the discrete eigenvalues to approximate continuous ones
    psi_j <- approxfun(t_grid, eigenvectors[, j], method = "linear", yleft = 0, yright = 0)
    
    # Now we can evaluate the j-th term at any s
    term <- (1 / j^q) * sapply(s, function(s_val) psi_j(s_val))
    
    # Sum up the terms
    beta_vals <- beta_vals + term
  }
  return(beta_vals)
}
################################################################## 
...
setPoIDGP <-
function(DGP_name){
if( DGP_name == "Easy2.110"){
         tau      <- c(0.166,0.333, 0.666,0.833) # PoI-locations tau_j
        beta_tau <- (10/sqrt(N))*c(-6,6,-5,5) # PoI-parameters
        fct_text <- "beta(t,2.1)"
        
    } 
	
  else if( DGP_name == "Easy2.140"){
       tau      <- c(0.166,0.333, 0.666,0.833) # PoI-locations tau_j
        beta_tau <- (40/sqrt(N))*c(-6,6,-5,5) # PoI-parameters
        fct_text <- "beta(t,2.1)"
        
    }
 else if( DGP_name == "Easy2.160"){
       tau      <- c(0.166,0.333, 0.666,0.833) # PoI-locations tau_j
        beta_tau <- (60/sqrt(N))*c(-6,6,-5,5) # PoI-parameters
        fct_text <- "beta(t,2.1)"
        
    }

    
else if( DGP_name == "No PoI2.1"){
       tau      <- c(0,0,0,0) # PoI-locations tau_j
        beta_tau <- c(0,0,0,0)   # PoI-parameters
        fct_text <- "beta(t,2.1)"
   }
...
#################################################################
#####Simulate X from the exponential Brownian motion
############################################################
BM.SIM <- sapply( 1:N , function(i) {
  dt <-(b-a)/(p-1)
...
FunRegPoI <-
  function(Y, X_mat, grd, add.vars = NULL, maxPoI= 8, ...) {
    ...
    estObj
            ...
            PoISim      <- FunRegPoISim(N = N , grd, DGP = DGP_name , error_sd = sigma)
            
            EstKPS      <- FunRegPoI(Y = PoISim$Y , X_mat = PoISim$X , grd, estimator = "KPS",maxPoI = maxPoI, k_seq = k_seq,scaleSearchPoI = scaleSearchPoI, threshold_cpv = FALSE)
            
            
            if(is.null(EstKPS$coefficients$tauInd)==FALSE){
              
              index_tau    <-EstKPS$coefficients$tauInd
              
              Princi<-selectEFwithCumPropVar(X_mat = PoISim$X)
            
              k_sum       <- EstKPS$model$k
              thethatij<-(t(X_mat)%*%Princi$evecs[,1:min(k_sum,dim(Princi$evecs)[2])]) 
              if((ncol(as.matrix(Princi$evecs[index_tau,1:min(k_sum,dim(Princi$evecs)[2])])))==1){
                sumjkthetaphi<-thethatij%*%Princi$evecs[index_tau,1:min(k_sum,dim(Princi$evecs)[2])]/p
              } else
              {
                sumjkthetaphi<-thethatij%*%t(Princi$evecs[index_tau,1:min(k_sum,dim(Princi$evecs)[2])])/p
              }
              
              X_k_hat_tau<-X_mat[index_tau,]-t(sumjkthetaphi)
              S_hat <- EstKPS[1]$coefficients$selS
              ...
              beta_hat <- EstKPS[1]$coefficients$betaPoI
              numerator <- t(beta_hat)%*%M%*%beta_hat
              
              Y_hat <- matrix(EstKPS[3]$fitted,nrow=N)
              denominator <- t(Y_hat-PoISim$Y)%*%(Y_hat-PoISim$Y)/N
              T <- numerator/denominator 
              rej_H0<-0
              if(T>qchisq(1-(alpha), S_hat))
                rej_H0 <- rej_H0 + 1
              print(rej_H0)
              list( T, S_hat, rej_H0)
            }
                 ...
                 save(rej_h0_list ,file = paste0("4poi_s=",scalePoISearch,"-DGP=",DGP_name,"-N=",
                             N ,".RData"))
